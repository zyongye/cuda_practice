...
int main() {
  ...
  int blocks = ds / chunks;

  for (int i = 0; i < chunks; i++) { //depth-first launch
    cudaMemcpyAsync(d_x + blocks * i, h_x + blocks * i, blocks * sizeof(ft), cudaMemcpyHostToDevice, streams[i % num_streams]);
    gaussian_pdf<<<(blocks + 255) / 256, 256, 0, streams[i % num_streams]>>>(d_x + blocks * i, d_y + blocks * i, 0.0, 1.0, blocks);
    cudaMemcpyAsync(h_y + i * blocks, d_y + i * blocks, blocks * sizeof(ft), cudaMemcpyDeviceToHost, streams[i % num_streams]);
  }
  cudaDeviceSynchronize();
